...
`src/app/app.routes.ts` — the main flow below (all under `src/app/pages/`), plus `home` and static legal pages (`privacy`, `terms`, `affiliate-disclosure`); default redirect to `/builder`.
1. **`/builder`** (`BuilderComponent`) — the wizard. Step 1: pick appliances + seasons + ZIP. Computes live `peakWattage` and `totalWattHours`. On submit, fetches sun-hours, assigns a `uuid` to the build, saves it, and navigates to `/results?buildId=...`.
   - **`/results`** (`ResultsComponent`) — picks the anchor station for the build's peak/storage/solar targets, persists it, and shows ranked vendor bundles (or a synthesized kit); "Customize" hands off to `/build`.
...
`src/app/content/*.ts` export plain arrays: `appliances.ts`, `inverters.ts`, `batteries.ts`, `solarPanels.ts`, `product-bundle-offers.ts`, plus `strings.ts` for UI copy. Services and pages read gear through the **`CATALOG` injection token** (`content/catalog.ts`), not by importing those files, so specs can supply the frozen fixture in `src/testing/catalog-fixture.ts` (a price refresh never breaks a unit test; only `catalog-integrity.spec.ts` reads the live data). Products carry `availability` (sold-out/discontinued stations are never anchored or offered as step-ups; unpurchasable bundles are hidden) and a `vendorRef` (vendor variant id/SKU) for matching against vendor or affiliate feeds. Bundle offers are authored with the vendor's `packagePrice`; `resolveBundleOffers` (`content/offer-pricing.ts`) derives the quoted `price`, `presetPrice` and note. Use the `update-ogpp-products` skill for catalog work. **IDs are stable content-derived slugs** assigned at module-load time via `assignStableIds` (`catalog-utils.ts`) — e.g. an appliance's ID is `slug(applianceGroup-name)`, an inverter's is `slug(brand-name)`. Reordering/inserting/removing entries does **not** renumber the others, so builds persisted in `localStorage` keep mapping to the right item. Collisions get a numeric suffix. (Previously IDs were positional array indices — a data-integrity hazard, now removed.)
